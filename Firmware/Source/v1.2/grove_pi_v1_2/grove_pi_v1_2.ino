...
void loop()
{
  ...
  if(index==4 && flag==0)
  {
    ...
    if(cmd[0]==50)
    {
      ...
      if (cmd[1] < 255 && cmd[1] >= 0)
      {
        rcolor = cmd[1];
      }
      if (cmd[2] < 255 && cmd[2] >= 0)
      {
        gcolor = cmd[2];
      }
      if (cmd[3] < 255 && cmd[3] >= 0)
      {
        bcolor = cmd[3];
      }

      rgbstrip.begin(); // begin
      rgbstrip.SetColor(rcolor, gcolor, bcolor);
      rgbstrip.end();
    }
    // In case of input errors try to end the previous cycle
    if(cmd[0] == 51)
    {
      rgbstrip.end();
    }
